...
async function setUpDatabaseAndCollection(): Promise<void> {
    ...
    const db = client.db("invertory_database")
    const collections = await db.listCollections({ name: "items" }).toArray()

    if (collections.length === 0) {
        await db.createCollection("items")
        console.log(
            "Created 'items' collection in 'invertory_database' database."
        )
    } else {
        console.log(
            "'items' collection already exists in 'invertory_database' database."
        )
    }
}
